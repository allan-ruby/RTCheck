...
import numpy as np
from Core_Code.PredRetDatabaseProcessor import fps_plus_mw
import pickle
...
def Join_StandAlone_Molecular_Features(file):
    df = pd.read_csv(file)
    list_data_df = []
    for index,row in df.iterrows():
        name = row[0]
...
def Join_Mummichog_Results_Retention_Times(filedir):
    results = pd.read_csv(filedir + 'DiffReport.tsv',sep='\t',encoding = "utf-8")
    ...
    final_df = id_df.merge(results[['mzmed','rtmed','rtmin']],how='left',on='mzmed')
    if not os.path.exists(filedir + '/RT_Folder/'):
        os.makedirs(filedir + '/RT_Folder/')
...
def Join_Mummichog_Matches_Molecular_Features(result_df,file_dir):
    ...
    for index,row in result_df.iterrows():
        print("On Index {} of {}".format(index+1,len(result_df)))
        if index == 0:
    ...
    if not os.path.exists(file_dir + '/RT_Folder/'):
        os.makedirs(file_dir + '/RT_Folder/')
    finaldf.to_pickle(file_dir + 'RT_Folder\\mummichog_rt_features.p')
    return finaldf



def Run_Model_On_Punitive_Matches(mol_characteristics,rt_file,model_dir,file_dir,threshold):
    if 'System' in mol_characteristics.columns:
        mol_characteristics = mol_characteristics.drop(['System'],axis=1)
    if 'Unnamed: 0' in mol_characteristics.columns:
        mol_characteristics = mol_characteristics.drop(['Unnamed: 0'],axis=1)
...
#            test_list.append(test_name)
    #    print('On index {}'.format(index))
        ...
        actual_rt = round(row['ART'],10)
        test_df = essential_df
        ...
        cols_to_be_dropped_file = pd.read_csv('Settings/columnstoberemoved.csv')
        cols_to_be_dropped = [col for col in cols_to_be_dropped_file.columns]
        for col in cols_to_be_dropped:
            if col in essential_df.columns:
                essential_df = essential_df.drop([col],axis=1)
        ...
        for index,result in enumerate(predictions):
            ...
            if prob_A[index][0] > .45 and prob_A[index][0] < .55:
                print('bypass')
            elif result == 1 and beginning_range < rt < end_range:
                beginning_range = rt
            elif result == 0 and  rt < end_range:
                end_range = rt
        ...
        elif beginning_range-threshold < actual_rt < end_range+threshold and test_name not in list_of_approved_cmpds:
            list_of_approved_cmpds.append([test_name,beginning_range-threshold,end_range+threshold,actual_rt])
        elif not beginning_range-threshold < actual_rt < end_range+threshold and test_name not in list_of_unapproved_cmpds:
            list_of_unapproved_cmpds.append([test_name,beginning_range-threshold,end_range+threshold,actual_rt])
    ...
    print('Number of  unapproved compounds were {}, percentage is {}'.format(len(list_of_unapproved_cmpds),(len(list_of_unapproved_cmpds)/total) * 100))
    print('Number of approved compounds were {}, percentage is {}'.format(len(list_of_approved_cmpds),(len(list_of_approved_cmpds)/total) * 100))
    print('Number of unsure compounds were {}, percentage is {}'.format(len(list_of_unsure_cmpds),(len(list_of_unsure_cmpds)/total) * 100))
    if not os.path.exists(file_dir + '/RT_Folder/'):
        os.makedirs(file_dir + '/RT_Folder/')
    if len(list_of_approved_cmpds) > 0:
        write_list_to_csv(list_of_approved_cmpds,file_dir + 'RT_Folder/approved.csv')
    if len(list_of_unapproved_cmpds) > 0:
        write_list_to_csv(list_of_unapproved_cmpds,file_dir + 'RT_Folder/unapproved.csv')
    if len(list_of_unsure_cmpds) > 0:
...
def Compile_Results_By_RT(approved,unapproved,diff_report,out_dir):
    approved_df = pd.read_csv(approved,index_col=False)
    unapproved_df = pd.read_csv(unapproved,index_col=False)
...
def Run_Model_On_Punitive_Matches_debug(mol_characteristics,rt_file,model_dir,file_dir,threshold):
    if 'System' in mol_characteristics.columns:
        mol_characteristics = mol_characteristics.drop(['System'],axis=1)
    if 'Unnamed: 0' in mol_characteristics.columns:
        mol_characteristics = mol_characteristics.drop(['Unnamed: 0'],axis=1)
...
#            test_list.append(test_name)
    ...
    if len(list_of_unsure_cmpds) > 0:
        write_list_to_csv(list_of_unsure_cmpds,file_dir + '/RT_Folder/unsure.csv')
    return list_of_approved_cmpds,list_of_unapproved_cmpds,list_of_unsure_cmpds,predictions, prob_A,test_df
...
def One_Click_Program(file_dir,template_rt_file_dir,model_dir):
    ...
    approved,unapproved,unsure = Run_Model_On_Punitive_Matches(mummichog_with_rts_mol_features,template_rt_file_dir,model_dir,file_dir,1.75)
    
    Compile_Results_By_RT(file_dir + '/RT_Folder/approved.csv',file_dir + '/RT_Folder/unapproved.csv', file_dir + 'DiffReport.tsv',file_dir+ '/RT_Folder/')
    print('To run model took {}'.format(datetime.now() - time2))
    print('To run process took {}'.format(datetime.now()-time1))
